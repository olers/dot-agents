...
（顶部补 `import { writeFile, readFile } from 'node:fs/promises'`。若实测发现 `open` 包在 darwin 用绝对路径绕过 PATH 垫片，则删除 openLog 断言、保留其余三条，并在测试注释里写明原因 —— 不许静默留一个必红断言。）
...
! grep -q '不做常驻进程' README.md             # 旧绝对化表述零命中
```
