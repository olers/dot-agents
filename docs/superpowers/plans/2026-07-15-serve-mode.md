# serve 宿主模式 Implementation Plan（v4 · R3 已修订，执行前以本版为准）
...
- **解决什么**：dot-agents 现在只能「开一次用一次」，跑起来自动弹浏览器、关掉就没了；别的工具（比如 camp-d2c 门户）想把它的界面装进自己页面里，装不进。
- **为什么现在做**：门户已裁决用内嵌方式集成，这是唯一的前置依赖；同时现在的安装链路有个坑——别人通过 npm 从 git 装它会装出一个缺构建产物的坏包，不修谁也用不了。
- **打算怎么做**：加一个显式的「被托管」模式（serve）：安静待在指定门牌号上、生死归宿主管；同时把长驻带来的安全问题（恶意网页冒充本机偷操作权、透明层骗点确认按钮）一并堵死。
- **完成后什么样**：宿主一条命令就能把它拉起并嵌进自己页面；普通用户的默认用法一个字都不变。
- **影响是什么**：新增一个子命令和一个健康检查端点；所有请求多一道「来访者身份核对」；安装配置修正两处。
- **一句话**：给 dot-agents 加一个安全的被托管模式，并修好对外分发。
...
- **顶回 1 条**：R1 第 14 维要求入口拆到 ≤400 行 —— 那是 camp-d2c 仓库约定；本仓库走 superpowers 约定（代码完整内联、禁占位符），不拆，补导读代偿。R2 已同意顶回（行数不作硬条件）。
- **R2 → v3**：Task 4 lock 验收拆掉吞失败的 `|| true`，改逐条 fail-closed 断言；默认命令回归从「help 文本冒烟」升级为真跑无子命令路径（PATH 垫假 `open`），help 冒烟改名如实；首行 JSON 测试补「键集恰为 app/url/port」断言；SIGINT/SIGTERM 关闭失败改 exit 1；Task 5 README 补可执行验收；导读改六块 + 各 Task 补一行人话意图。
- **R3 → v4**：Task 4 / Task 5 的验收命令块补 `set -euo pipefail`（整块真 fail-closed，不靠人工逐条）；README 负断言改为命中真实旧句 `server 跑完即退`；删除 openLog 断言的「降级口」——断言必留，shim 不生效只许修 shim 或换可测接缝，不许删；标题与版本标识升 v4。**包名已改 `@linemagic/dot-agents`**：bin 名仍为 `dot-agents`，Task 4 的 lock 断言与 consumer 验证不受影响；healthz `app` 字段固定 `dot-agents` 不随包名走。R3 判定：3 PASS / 5 REVISE（本版逐条吸收，涉及门户侧的 SIGTERM 主线程问题在门户 plan v4 修）。
...
**意图（人话）**：给服务加一道「来访者报的门牌号必须是本机」的核对，堵死恶意网页冒充本机偷操作权的路；再开一个「你是谁、看着哪个仓库」的问询口，让宿主能认人。
...
**意图（人话）**：让服务能按宿主指定的门牌号开门（占了就明说，绝不偷偷换号），并且只允许指定的页面把它嵌进去，防透明层骗点确认按钮。
...
**意图（人话）**：把「被托管模式」做成一条命令：起来第一句话就用固定格式报出自己在哪，之后安静干活，宿主喊停就体面退出；起不来就明确报错退出，绝不含糊。
...
describe('默认命令回归（真跑无子命令路径）', () => {
  ...
  it('无子命令：打启动行、healthz 通、SIGINT 退出 0、调用了 open', async () => {
    ...
    // open 是异步 fire-and-forget，给它一点时间落 log
    await new Promise((r) => setTimeout(r, 500))
    expect((await readFile(openLog, 'utf8'))).toContain(url!)
    proc.kill('SIGINT')
...
（顶部补 `import { writeFile, readFile } from 'node:fs/promises'`。**openLog 断言不许删** —— 它是「open 被调用」这条默认行为的唯一证明。若实测 shim 不生效，只许两条路：修 shim（查 chmod / env PATH 透传 / spawn 解析），或把 open 调用抽成 CLI 内可注入的接缝再测；两条都走不通才回 plan 修订，不许静默降级。）
...
**意图（人话）**：修好「别人从 git 安装会装出缺构建产物的坏包」的问题，并放宽运行环境要求到主流版本，让消费方装完即可用。
...
整块直接复制执行（`set -euo pipefail` 保证任何一条失败整块即失败，不靠人工逐条盯）：

```bash
set -euo pipefail
npm install --package-lock-only
grep -q '"node": ">=18"' package-lock.json     # engines 已降
! grep -q '"agents": "\./dist' package-lock.json   # 旧 bin 名零命中
grep -q '"dot-agents": "\./dist' package-lock.json # 新 bin 名在（bin 名不随包名 @linemagic/dot-agents 走）
echo LOCK-OK
```
...
**意图（人话）**：把新模式和安全边界写进说明书，并改掉「本工具从不长驻」这句已经不再成立的旧话，防止文档骗人。
...
```bash
set -euo pipefail
grep -q '宿主模式（serve）' README.md          # 新章节在
grep -q 'allow-embed' README.md                # 契约关键词在
! grep -q 'server 跑完即退' README.md          # 真实旧句（原 README.md:55）零命中
echo README-OK
```

Expected: 输出 `README-OK`。若仓库存在 `README.zh-CN.md` 等多语言版，同句同改同验。
